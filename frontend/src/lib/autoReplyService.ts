...
export const generateEnhancedAutoReply = async (threadId: string) => {
  try {
    ...
    if (SKIP_CATEGORIES.includes(primaryCategory)) {
      console.log(
        `Skipping auto-reply generation for ${primaryCategory} email`
      );
      return {
        data: {
          status: "skipped",
          message: `Email classified as ${primaryCategory} - skipping auto-reply`,
        },
      };
    }
 ...
 */
export const triggerEnhancedAutoReply = async (
  maxResults = 20,
  useHtml = false
) => {
