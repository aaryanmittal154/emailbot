...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("background_service.log"), logging.StreamHandler()],
)
...
def log_background_service_event(user_id, event_type, status, details=None, db=None):
    ...
    try:
        ...
        # Serialize the details dictionary to a JSON string
...
class BackgroundService:
    ...
    def process_all_users(self):
        ...
        try:
            ...
            if active_count == 0:
                logger.info(
                    "No users with background service enabled, skipping processing cycle"
                )
                return
            ...
            for user in users:
                try:
                    ...
                    if not self.verify_user_service_eligibility(user["id"]):
                        logger.info(
                            f"User {user['id']} does not meet service eligibility, skipping"
                        )
                        continue
    ...
    def process_user(self, user: Dict[str, Any]):
        ...
        if not self.verify_user_service_eligibility(user_id):
            logger.info(
                f"User {user_id} is no longer eligible for background service, skipping"
            )
            return
        ...
        try:
            ...
            new_emails = self.email_service.get_unreplied_emails(
                user_id=user_id, access_token=access_token, max_results=20
            )
            ...
            for email in new_emails:
                try:
                    ...
                    if self.has_reached_daily_limit(user_id, user["max_daily_emails"]):
                        logger.info(
                            f"User {user_id} reached daily limit during processing, stopping"
                        )
                        break
                    ...
                    logger.error(
                        f"Error processing email {email['thread_id']} for user {user_id}: {str(e)}"
                    )
    ...
    def process_email(
        self, user_id: str, email: Dict[str, Any], access_token: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            reply = self.email_service.generate_auto_reply(
                thread_id=thread_id, context=context, user_id=user_id
            )
            ...
            log_background_service_event(
                ...
                status="success",
                details={"thread_id": thread_id, "category": primary_category},
                db=self.db,
            )
            ...
            return {"thread_id": thread_id, "reply_sent": False, "error": str(e)}

    def gather_cross_category_context(
        self, thread_id: str, primary_category: str
    ) -> List[Dict[str, Any]]:
    ...
    def is_user_enabled(self, user_id: int) -> bool:
        ...
        result = self.db.query_one(query, [user_id])
        return (
            result["background_enabled"]
            if result and "background_enabled" in result
            else False
        )
