...
async def similar_threads_multi(
    thread_id: str = Query(..., description="The thread ID to find similar threads for"),
    include_categories: str = Query("", description="Comma-separated list of categories to include"),
    exclude_categories: str = Query("", description="Comma-separated list of categories to exclude"),
    ...
    try:
        ...
        include_list = [cat.strip() for cat in include_categories.split(",")] if include_categories else []
        exclude_list = [cat.strip() for cat in exclude_categories.split(",")] if exclude_categories else []
...
async def check_new_emails_enhanced(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error checking new emails: {str(e)}"
        )
...
def generate_context_for_email(
    ...
    try:
        ...
        if use_cross_category:
            ...
            context["cross_category_matches"] = [format_search_result(r) for r in cross_results]
