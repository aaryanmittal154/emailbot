...
async def get_status(
    ...
    try:
        ...
        try:
            ...
            query = text(
                "SELECT 1 FROM tokens WHERE user_id = :user_id AND refresh_token IS NOT NULL"
            )
            ...
            if not has_token:
                ...
                query = text(
                    "SELECT 1 FROM oauth_tokens WHERE user_id = :user_id AND refresh_token IS NOT NULL"
                )
                token_result = db.execute(query, {"user_id": user_id}).fetchone()
                has_token = token_result is not None
...
async def pause_service(
    ...
    success = update_background_preferences(
        user_id=user_id, background_enabled=False, db=db
    )
...
async def resume_service(
    ...
    # Update preferences in database
    success = update_background_preferences(
        user_id=user_id, background_enabled=True, db=db
    )
...
async def toggle_background_service(
    ...
    try:
        ...
        query = text(
            "SELECT background_enabled FROM user_background_preferences WHERE user_id = :user_id"
        )
        result = db.execute(query, {"user_id": user_id}).fetchone()
        ...
        if not current_status:  # If we're enabling the service
            ...
            if not has_token:
                query = text(
                    "SELECT refresh_token FROM oauth_tokens WHERE user_id = :user_id"
                )
                token_result = db.execute(query, {"user_id": user_id}).fetchone()
                has_token = token_result is not None and token_result[0] is not None
        ...
        success = update_background_preferences(
            user_id=user_id, background_enabled=new_status, db=db
        )
        ...
        return {"success": False, "message": f"Error toggling service: {str(e)}"}
...
async def background_service_oauth_callback(
    ...
    try:
        ...
        backend_url = "https://emailbot-k8s7.onrender.com"
        ...
        frontend_url = (
            f"{settings.CORS_ORIGINS[0]}/settings/background-service?enabled=true"
        )
...
async def test_oauth_flow():
    ...
    try:
        ...
        logger.info(
            f"Registered redirect URIs in settings: {settings.GOOGLE_REDIRECT_URI}"
        )
